...
function flatten(data) {
    ...
    res = []
    // return data.map( arr => {return res.concat(flatten(arr))})
   for (let arr of data) {
...
function fileFinder(directories, targetFile) {
    ...
    for (let dir in directories) {
            // console.log(directories[dir] , "dir2");
           if (dir === targetFile || fileFinder(directories[dir], targetFile)) return true;
    }

    // for (let dir in directories) {
    //     if (dir[0] === "/") {
    //         if (fileFinder(directories[dir], targetFile)) return true
    //     } else {
    //         if (dir === targetFile) return true;
    //     }
    // }
    // return false;
}
...
function pathFinder(directories, targetFile) {
    ...
    for (let file in directories) {
        let isDir = file[0] === "/"
        if (isDir) {
            let res = pathFinder(directories[file], targetFile)
            if (res) return  file + res;
        } else {
