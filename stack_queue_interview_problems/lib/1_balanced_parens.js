...
// Let's code!
// -----------
// if the first is ")"; or last is "("; or length is not even; return false
// then loop through the str; taking the first and last elements; they should be a correct pair 
function balancedParens(str) {
    ...
    let strs = str.split("");
    let strList = strs.filter(cha => brackets.includes(cha));
    let openparts = [];
    ...
    for (let el of strList) {
        if (openBrackets.includes(el)) {
            ...
            let top = openparts.pop();
            if (bracketpairs[el] !== top) return false
        }
    }
    
    return openparts.length === 0;


    // let first = strList.shift();
    // let last = strList.pop();
    // if (str.length === 0 ) {
    //     return true;
    // } else if ( strList.length % 2 != 0 || !bracketPairs.includes(first + last)) {
    //     return false;
    // } else {
    //     while (strList.length > 0) {
    //        first = strList.shift();
    //        last = strList.pop();
    //        if (strList.length === 0 && (first + last === "()" || first + last === ")("
    //                                     || first + last === "{}" || first + last === "}{")
    //                                     || first + last === "[]" || first + last === "][") return true;
