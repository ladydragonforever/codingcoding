...
//         NonAdjacentNeighbor = tab[i-2] === undefined ? 0 : tab[i-2];
//         includeThisNumber = NonAdjacentNeighbor + nums[i];
//         notIncludeThisNumber = tab[i-1];
//         tab[i] = Math.max(includeThisNumber, notIncludeThisNumber);
//     }
//     return tab[nums.length - 1]
...
function maxNonAdjacentSum(nums, memo = {}) {
    ...
    memo[nums.length] = Math.max(first+ maxNonAdjacentSum(nums.slice(2), memo), maxNonAdjacentSum(nums.slice(1), memo))
